...
public class Runigram {
	...
	private static Color luminance(Color pixel) {
		...
		Color newP = new Color((int) (lum), (int) (lum), (int) (lum));
		return newP;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double heightR = ((double) image.length / height);
		double widthR = ((double) image[0].length / width);
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				newImg[i][j] = image[(int) (heightR * i)][(int) (widthR * j)];
			}
		}
		return newImg;
	}
